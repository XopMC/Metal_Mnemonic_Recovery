...
void printHelp() {
    std::cout
        << metal_mnemonic_recovery::kProjectName << " " << metal_mnemonic_recovery::kProjectVersion << "\n"
        << "Recovery-only Metal/macOS tool for restoring missing BIP39 mnemonic words.\n\n"
        << "Usage:\n"
        << "  Metal_Mnemonic_Recovery -recovery \"word1 word2 * ...\" -d derivations.txt [options]\n"
        << "  Metal_Mnemonic_Recovery -recovery -i templates.txt -d derivations.txt [options]\n\n"
        << "Recovery sources:\n"
        ...
        << "                       3=check both, 4=ed25519-bip32.\n"
        << "                       If omitted, the tool uses the usual derivation style\n"
        ...
        << "  -xx FILE             XOR verification filter (.xor_u).\n"
        << "  -xb FILE             Bloom verification filter.\n\n"
        << "Runtime options:\n"
        << "  -pbkdf N             Override PBKDF2 iterations. Default: 2048.\n"
        << "  -pass VALUE|FILE     Single passphrase or passphrase file.\n"
        << "  -device LIST         Metal device selector. Compatibility accepts lists or ranges,\n"
        << "                       but the current public runtime uses the first valid device.\n"
        << "  -b N                 Metal dispatch group count.\n"
        << "  -t N                 Metal threads per threadgroup.\n"
